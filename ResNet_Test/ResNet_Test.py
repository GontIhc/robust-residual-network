...
# 定义模型
myModel = torchvision.models.resnet101(pretrained=Fa)
...
for _epoch in range(10):
    ...
    for _step, input_data in enumerate(train_loader):
        ...
        if _step % 10 == 0:
            print('[iteration - %3d] training loss: %.3f\tbatchSum：%d' % (_epoch * len(train_loader) + _step, training_loss / 10, len(train_loader)))
            training_loss = 0.0
            print()
    correct = 0
    total = 0
